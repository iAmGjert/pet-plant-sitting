...
const MoreInfo = (props) => {
  ...
  const postApplication = async (newApplicant: any) => {
    return await axios
      .post('/api/jobs/applicant/create', newApplicant)
      .then((res: any) => {
        return res;
      })
      .catch((err) => {
    ...
    >
      ...
      <Modal.Body className='show-grid'>
        <Container>
          ...
          <Row>
            From {moment(job.startDate).format('MM/DD')} to{' '}
            {moment(job.endDate).format('MM/DD')}.
          </Row>
          <Row>
            Job length:{' '}
            {moment(job.endDate).diff(moment(job.startDate), 'days')}{' '}
            {moment(job.endDate).diff(moment(job.startDate), 'days') > 1
              ? 'days'
              : 'day'}
          </Row>
          {employer !== user.name ? (
            <Row>
              {distance === null
                ? 'Add a location to your profile to see the distance between you and this job!'
                : `Distance to job: ${distance} miles`}
            </Row>
          ) : (
            <Row>
